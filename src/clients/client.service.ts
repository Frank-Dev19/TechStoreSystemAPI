...
export class ClientService {
  ...
  private inferClientKindFromImportRow(
    row: NormalizedImportRow,
    documentType?: DocumentType,
  ): ClientKind {
    if (documentType?.kind === DocumentTypeKind.COMPANY) {
      return ClientKind.COMPANY;
    }

    if (documentType?.kind === DocumentTypeKind.PERSON) {
      return ClientKind.PERSON;
    }

    const normalizedName = String(documentType?.name ?? '').toUpperCase();
    if (normalizedName.includes('RUC') || Number(documentType?.digits) === 11) {
      return ClientKind.COMPANY;
    }

    return this.inferClientKind(row.documentNumber);
  }
  private normalizeContactInput(
    contact: ClientContactInputDto,
  ): NormalizedClientContactInput {
    return {
      ...
      isActive:
        contact.isActive !== undefined ? Boolean(contact.isActive) : true,
    };
  }
  ...
  private normalizePhoneOrThrow(
    ...
    if (!normalized) {
      throw new BadRequestException(
        `${fieldLabel} must be a valid E.164 phone number`,
      );
    }
  ...
  private async saveClientContacts(
    ...
    if (!normalizedContacts.length) {
      throw new BadRequestException(
        'Company clients require at least one contact',
      );
    }
  ...
  private async validateImportRows(
    ...
    const documentTypeIds = Array.from(
      new Set(
        normalizedRows
          .map((row) => Number(row.documentTypeId))
          .filter((value) => value > 0),
      ),
    );

    const documentTypes = documentTypeIds.length
      ? await this.documentTypeRepository.find({
          where: { id: In(documentTypeIds) },
        })
      : [];

    const documentTypesById = new Map(
      documentTypes.map((docType) => [Number(docType.id), docType]),
    );
    const existingClientsByKey = await this.buildExistingClientsMap(
      companyId,
      normalizedRows,
    );
    ...
    return normalizedRows.map((row) => {
      const errors: string[] = [];
      const docTypeId = row.documentTypeId
        ? Number(row.documentTypeId)
        : undefined;
      ...
      if (!row.documentNumber) {
        ...
        errors.push(
          'El número de documento no puede ser 0 ni contener solo ceros.',
        );
      } else if (
        docType &&
        row.documentNumber.length !== Number(docType.digits)
      ) {
      ...
      const duplicateKey =
        docTypeId && row.documentNumber
          ...
          : undefined;
      const duplicateExistingClient = duplicateKey
        ? existingClientsByKey.get(duplicateKey)
        : undefined;

      if (duplicateExistingClient) {
        errors.push(
          'Ya existe un cliente con ese documento en la base de datos.',
        );
      }
  ...
  private async validateDocumentLength(
    documentTypeId: number,
    documentNumber: string,
  ) {
    ...
    if (documentNumber.length !== dt.digits) {
      throw new BadRequestException(
        `Document number must be ${dt.digits} digits`,
      );
    }
  }
  ...
  async create(createClientDto: CreateClientDto) {
    ...
    await this.validateDocumentLength(
      createClientDto.documentTypeId,
      createClientDto.documentNumber,
    );
    ...
    const kind =
      createClientDto.kind ??
      this.inferClientKind(createClientDto.documentNumber);
    const normalizedPhone = this.normalizePhoneOrThrow(
    ...
    const saved = await this.clientRepository.manager.transaction(
      async (manager) => {
        ...
        const transactionClientContactRepository =
  ...
  async findAll(query: FindAllQuery) {
    ...
    if (!query.companyId || Number.isNaN(companyId) || companyId <= 0) {
      throw new BadRequestException(
        'Valid companyId is required to list clients',
      );
    }
    ...
    const normalizedStatus = (query.status ?? 'active')
      ...
      .toLowerCase();
    const showDeleted =
      normalizedStatus === 'deleted' || normalizedStatus === 'eliminados';
    const deletedCondition = showDeleted ? Not(IsNull()) : IsNull();
    const documentTypeId =
      query.documentTypeId != null ? Number(query.documentTypeId) : undefined;

    const baseCondition: Pick<
    ...
    const buildCondition = (
      condition: FindOptionsWhere<Client>,
    ): FindOptionsWhere<Client> => ({
      ...condition,
      companyId: baseCondition.companyId,
      deletedAt: baseCondition.deletedAt,
      ...(documentTypeId && !Number.isNaN(documentTypeId)
        ? { documentTypeId }
        : {}),
    });
  ...
  async findOne(id: number) {
    ...
    if (Array.isArray(client.contacts)) {
      client.contacts = [...client.contacts].sort(
  ...
  async update(id: number, updateClientDto: UpdateClientDto) {
    ...
    const kind =
      updateClientDto.kind ??
      client.kind ??
      this.inferClientKind(
        updateClientDto.documentNumber ?? client.documentNumber,
      );
    ...
    Object.assign(client, {
      ...
      requiresContactCompletion:
        kind === ClientKind.PERSON ? false : client.requiresContactCompletion,
    });
    const saved = await this.clientRepository.save(client);
    if (kind === ClientKind.PERSON) {
      await this.clientContactRepository.delete({ clientId: saved.id });
      saved.requiresContactCompletion = false;
      await this.clientRepository.save(saved);
    } else if (contacts) {
  ...
  async validateImport(dto: ValidateClientImportDto) {
    ...
    if (!companyId || Number.isNaN(companyId)) {
      throw new BadRequestException(
        'Valid companyId is required to validate import rows',
      );
    }
  ...
  async commitImport(dto: CommitClientImportDto) {
    ...
    if (!companyId || Number.isNaN(companyId)) {
      throw new BadRequestException(
        'Valid companyId is required to import clients',
      );
    }

    const validatedRows = await this.validateImportRows(
      companyId,
      dto.rows ?? [],
    );
    ...
    if (readyRows.length) {
      const importDocumentTypeIds = Array.from(
        new Set(
          readyRows
            .map((row) => Number(row.documentTypeId))
            .filter((value) => value > 0),
      ...
      const importDocumentTypes = importDocumentTypeIds.length
        ? await this.documentTypeRepository.find({
            where: { id: In(importDocumentTypeIds) },
          })
        : [];
      const importDocumentTypesById = new Map(
        importDocumentTypes.map((docType) => [Number(docType.id), docType]),
      );

      const entities = readyRows.map((row) => {
        const documentType = row.documentTypeId
          ? importDocumentTypesById.get(Number(row.documentTypeId))
          : undefined;
        const kind = this.inferClientKindFromImportRow(row, documentType);
        const requiresContactCompletion = kind === ClientKind.COMPANY;
      ...
      try {
        const saved = await this.clientRepository.save(entities, {
          chunk: 200,
        });
