...
function getItemCode(item: SaleItem): string | null {
  if (item.itemType === 'SERVICE') return item.serviceCodeSnapshot ?? item.service?.code ?? null;
...
function getItemDescription(item: SaleItem): string {
  ...
  if (item.itemType === 'SERVICE') return item.serviceNameSnapshot ?? item.service?.name ?? 'SERVICIO';
